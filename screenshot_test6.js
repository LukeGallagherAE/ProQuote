...
(async () => {
  ...
  // Load once to get the right localStorage context
  await page.goto('http://localhost:3000/', {waitUntil: 'domcontentloaded'});
  // Set fake auth token to bypass login
  await page.evaluate(() => {
    // Set a valid-looking JWT structure (header.payload.signature)
    const fakeJwt = 'eyJhbGciOiJIUzI1NiJ9.eyJ1aWQiOjEsImVtYWlsIjoibHVrZUBhZG9uYWllbGVjdHJpY2FsLmNvbSJ9.fake_signature';
    window._nativeLS.setItem('pq_jwt', fakeJwt);
    window._nativeLS.setItem('proquote_profile', JSON.stringify({name:'Luke',company:'Adonai Electrical',abn:'12 345 678 901',phone:'0407769296',email:'luke@adonaielectrical.com',labourSell:100,labourCost:75,markup:100,deposit:20}));
  });
  
  // Reload with auth set
  ...
  const state = await page.evaluate(() => {
    ...
    const setupWiz = document.getElementById('setup-wizard');
    return {
      landingVisible: landing ? landing.style.display !== 'none' : 'missing',
      setupVisible: setupWiz ? setupWiz.style.display !== 'none' : 'missing',
      hasS: typeof S !== 'undefined',
      hasFullRender: typeof fullRender === 'function'
    };
  });
  console.log('state:', state);
  
  if(state.landingVisible || state.setupVisible) {
    // Force dismiss
    await page.evaluate(() => {
      const l = document.getElementById('landing'); if(l) l.style.display='none';
      const s = document.getElementById('setup-wizard'); if(s) s.style.display='none';
  ...
  await new Promise(r => setTimeout(r, 800));
  await page.screenshot({path: '/tmp/pq_v4_main.png'});
  
  await page.evaluate(() => { try { openSubitemPanel('1', '2'); } catch(e){console.error(e)} });
  await new Promise(r => setTimeout(r, 600));
  await page.screenshot({path: '/tmp/pq_v4_panel.png'});
