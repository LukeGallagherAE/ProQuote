...
(async () => {
  ...
  const result = await page.evaluate(() => {
    try {
      ...
      if(wiz && wiz.style.display !== 'none') {
        localStorage.setItem('proquote_profile', JSON.stringify({name:'Luke',company:'Adonai Electrical',abn:'12345',phone:'0400000000',email:'luke@test.com',labourSell:100,labourCost:75,markup:100,deposit:20}));
        wiz.style.display = 'none';
      }
      
      // Small wait then inject
      if(typeof S === 'undefined' || typeof fullRender !== 'function') return 'not ready: ' + typeof S + ' ' + typeof fullRender;
  ...
  await page.evaluate(() => { try { openSubitemPanel('1', '2'); } catch(e){console.log(e.message)} });
