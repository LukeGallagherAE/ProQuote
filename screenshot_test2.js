...
(async () => {
  ...
  // Inject test data directly into app state
  const result = await page.evaluate(() => {
    try {
      // Try to dismiss any wizard/onboarding
      const btns = Array.from(document.querySelectorAll('button'));
      const skipBtn = btns.find(b => /skip|later|dismiss/i.test(b.textContent));
      if(skipBtn) skipBtn.click();
      
      // Force inject test data
      if(typeof S !== 'undefined' && typeof fullRender === 'function') {
        window._id = 100;
        S.sections = [{
          id:1, name:'Electrical',
          parents:[
            {id:2,name:'Dedicated 32a Circuit',description:'Install 32a circuit',qty:1,discount:0,optional:false,lumpSum:false,category:'Power',activeCats:{supply:true,wire:true,install:true},
             subitems:[
        ...
        window._activeSectionId = 1;
        if(typeof switchSbTab === 'function') switchSbTab('sections');
        fullRender();
        return 'ok';
      }
      return 'S not defined: ' + typeof S;
    } catch(e) { return 'error: ' + e.message; }
  });
  console.log('inject result:', result);
  
  await new Promise(r => setTimeout(r, 1000));
  await page.screenshot({path: '/tmp/pq_layout_v2.png', fullPage: false});
  
  // Open subitem panel
  await page.evaluate(() => {
    try { openSubitemPanel('1', '2'); } catch(e) {}
  });
  await new Promise(r => setTimeout(r, 600));
  await page.screenshot({path: '/tmp/pq_subitems_v2.png', fullPage: false});
